# PING是什么
很多人都知道PING这个程序，在你决定要看看某台机器是不上能连通的时候会使用PING这个命令，比如像下面这样：

```bash
ping 192.168.1.1
```

PING这个名字源于声纳定位，是由Mike Muuss编写的，目的是为了测试另一台主机是否可达，它会发送ICMP回显请求报文给目的主机，并期望得到ICMP回显应答报文。

一般来说，如果我们不能PING通某台主机，就不能Telnet到那台主机，但随着安全防火墙等的出现，很多时候PING不通某台主机很有可能是被对方机拦截了PING，而Telnet依然可以到达那台主机，所以PING不PING得通已经不再是机器是否可达的唯一标准了。
